...
import { Newspaper, Calendar, ArrowRight, TrendingUp, Search, Filter, LayoutGrid, Clock, ChevronRight, Zap } from "lucide-react"
...
export default async function NewsListingsPage({
  ...
  return (
    <div className="flex flex-col bg-background selection:bg-brand-blue/30 overflow-x-hidden">
       <header className="relative pt-48 pb-24 bg-slate-50 dark:bg-black/20 overflow-hidden">
          ...
          <div className="container relative z-10 mx-auto px-6 md:px-12">
             <div className="max-w-3xl space-y-10 animate-in fade-in slide-in-from-bottom-12 duration-1000">
                <div className="flex items-center gap-4">
                   ...
                   <span className="text-xs font-black text-brand-blue uppercase tracking-[0.4em]">Official Broadcast Stream</span>
                </div>
                <h1 className="text-[clamp(2.8rem,7vw,5.5rem)] font-black text-foreground dark:text-white leading-[0.95] tracking-tighter">
                   National Health <br />
                   <span className="text-brand-blue">{isBn ? 'সংবাদ।' : 'Bulletins.'}</span>
                </h1>
                <p className="text-lg md:text-xl font-medium text-muted-foreground dark:text-gray-400">
                   Real-time updates, official policy disclosures, and operational announcements from the DGHS Management Information System.
                </p>
             </div>
          </div>
       </header>
                  ...
                  <div className="relative rounded-[4.5rem] bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 overflow-hidden shadow-2xl transition-all duration-700 hover:shadow-brand-blue/10">
                     <div className="grid grid-cols-1 lg:grid-cols-2">
                        <div className="aspect-[4/3] lg:aspect-auto relative overflow-hidden">
                           {topNews.coverImage ? (
                             <CloudImage src={topNews.coverImage} alt={isBn ? topNews.titleBn : topNews.titleEn} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-1000" />
                           ) : (
                           ...
                           <div className="absolute top-10 left-10">
                              <span className="px-6 py-2.5 bg-brand-blue text-white text-[10px] font-black uppercase tracking-widest rounded-full shadow-2xl">
                                 Featured Headline
                              </span>
                           </div>
                        </div>
                        <div className="p-12 md:p-20 flex flex-col justify-center space-y-10">
                           ...
                           <div className="flex items-center gap-4 text-brand-blue font-black uppercase text-xs tracking-widest group-hover:gap-8 transition-all">
                              Continue Reading <ArrowRight className="h-5 w-5" />
                           </div>
                        </div>
                     </div>
                  </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 sm:gap-16">
                {remainingNews.map((news) => (
                  <Link key={news.id} href={`/${lang}/news/${news.id}`} className="group flex flex-col space-y-8">
                     <div className="aspect-[16/10] rounded-[3rem] overflow-hidden bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 group-hover:shadow-2xl transition-all duration-700">
                        {news.coverImage ? (
                          ...
                          <div className="h-full w-full bg-slate-50 dark:bg-white/5 flex items-center justify-center text-slate-200 dark:text-white/5">
       ...
       <section className="py-24 bg-slate-50/50 dark:bg-black/20 border-t border-slate-100 dark:border-white/5 h-fit text-center md:text-left">
          <div className="container mx-auto px-6 md:px-12 p-16 rounded-[4rem] bg-[#050f1e] text-white flex flex-col md:flex-row items-center justify-between gap-12 relative overflow-hidden shadow-2xl">
             ...
             <div className="relative z-10 space-y-6">
                <h2 className="text-3xl font-black tracking-tight leading-none">Operational Directives Registry</h2>
                <p className="text-white/40 text-lg font-medium max-w-xl">Archival history of all national health Information Stream bulletins since the start of digital transformation.</p>
             </div>
             <Link href={`/${lang}/contact`} className="relative z-10">
                <Button size="lg" className="rounded-full h-18 px-12 bg-white text-black hover:bg-brand-blue hover:text-white font-black text-sm uppercase tracking-widest transition-all shadow-[0_20px_50px_rgba(255,255,255,0.1)]">
                   Contact Command
                </Button>
             </Link>
          </div>
